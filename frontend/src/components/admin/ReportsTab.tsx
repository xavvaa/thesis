...
const convertToCSV = (reportData: any): string => {
  ...
  // Header section
  csvContent += `"PESO - Public Employment Service Office"\n`;
  csvContent += `"Report: ${reportMetadata.reportType}"\n`;
  ...
  if (data.summary) {
    ...
    Object.entries(data.summary).forEach(([key, value]) => {
      ...
      csvContent += `${escapeCSV(formattedKey)},${escapeCSV(value)}\n`;
    });
    csvContent += "\n";
  }
  
  // Detailed data with proper formatting
  if (data.details && Array.isArray(data.details) && data.details.length > 0) {
    ...
    csvContent += formattedHeaders.map(h => escapeCSV(h)).join(',') + '\n';
    ...
    // Add summary footer
    csvContent += `\n"Total Records: ${data.details.length}"\n`;
  }
